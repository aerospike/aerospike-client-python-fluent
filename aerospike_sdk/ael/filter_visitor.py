...
from aerospike_sdk.ael.filter_gen import (
    ExpressionNode,
    IndexTypeEnum,
    OperationType,
)
...
class FilterTreeVisitor(ConditionVisitor):
    ...
    def __init__(self, ael_string: str, placeholder_values: Optional[Any] = None):
        self._ael = ael_string
...
def build_filter_tree_from_parse_tree(
    parse_tree: Any, ael_string: str, placeholder_values: Optional[Any] = None
) -> Optional[ExpressionNode]:
    """Build ExpressionNode tree from ANTLR parse tree. Returns None if not filterable."""
    visitor = FilterTreeVisitor(ael_string, placeholder_values)
    try:
        return visitor.visit(parse_tree)
    except (AelParseException, ValueError, TypeError):
        return None
