...
class UdfFunctionBuilder:
    ...
    Produced by :meth:`~aerospike_fluent.aio.session.Session.execute_udf` or
...
class UdfBuilder:
    ...
    See Also:
        :meth:`~aerospike_fluent.aio.session.Session.execute_udf`: Entry point.
    """
    ...
    def where(
        ...
        See Also:
            :meth:`QueryBuilder.where`: Same DSL for reads.
        """
    ...
    async def execute(self, on_error: OnError | None = None) -> RecordStream:
        """Run the current builder state and return a :class:`~aerospike_fluent.record_stream.RecordStream`.
