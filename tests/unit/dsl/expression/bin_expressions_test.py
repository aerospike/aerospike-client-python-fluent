...
"""Unit tests for AEL bin expressions."""
...
class TestBinExpressions:
    ...
    def test_bin_gt(self):
        ...
        result = parse_ael("$.A > 10")
        assert result == expected

    def test_bin_ge(self):
        """>= int and string."""
        expected = Exp.ge(Exp.int_bin("A"), Exp.int_val(100))
        result = parse_ael("$.A >= 100")
        assert result == expected
        result = parse_ael("$.name >= 'text'")
    ...
    def test_bin_not_equals(self):
        ...
        result = parse_ael("$.A != 1")
        assert result == expected
        result = parse_ael("$.strBin != 'yes'")
        assert result == Exp.ne(Exp.string_bin("strBin"), Exp.string_val("yes"))

    def test_negative_string_bin_equals_unquoted_raises(self):
        """$.strBin == yes (unquoted) raises AelParseException."""
        with pytest.raises(AelParseException, match=r"operand|mismatched input|line \d+:\d+"):
            parse_ael("$.strBin == yes")
    ...
    def test_reversed_operands_literal_on_left(self):
        ...
        result = parse_ael("'text' < $.stringBin1")
