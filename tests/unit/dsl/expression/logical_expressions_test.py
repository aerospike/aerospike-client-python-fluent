...
"""Unit tests for AEL logical expressions."""
...
class TestLogicalExpressions:
    ...
    def test_bin_logical_and_or_combinations(self):
        ...
        result = parse_ael("$.intBin1 > 100 and $.intBin2 > 100")
        ...
        result = parse_ael("$.intBin1 > 100 and $.intBin2 > 100 or $.intBin3 < 100")
        ...
        result_wrong_grouping = parse_ael(
    ...
    def test_or_expression(self):
        ...
        result = parse_ael("$.A == 1 or $.A == 2")
        assert result == expected

    def test_negative_syntax_logical_operators(self):
        """Malformed logical expressions raise AelParseException."""
        with pytest.raises(AelParseException, match=r"Could not parse|parse|line \d+:\d+|mismatched|extraneous"):
            parse_ael("($.intBin1 > 100 and ($.intBin2 > 100) or)")

        with pytest.raises(AelParseException, match=r"Could not parse|parse|line \d+:\d+|mismatched|extraneous"):
            parse_ael("and ($.intBin1 > 100 and ($.intBin2 > 100))")

        with pytest.raises(AelParseException, match=r"Could not parse|parse|line \d+:\d+|mismatched|extraneous"):
            parse_ael("($.intBin1 > 100 and ($.intBin2 > 100) not)")

        with pytest.raises(AelParseException, match=r"Could not parse|parse|line \d+:\d+|mismatched|extraneous"):
            parse_ael("($.intBin1 > 100 and ($.intBin2 > 100) exclusive)")

    def test_negative_bin_logical_exclusive_with_one_param(self):
        """exclusive() with one expression raises AelParseException (grammar or visitor)."""
        with pytest.raises(
            AelParseException,
            match=r"at least 2 expressions|mismatched input.*expecting ','|no viable alternative",
        ):
            parse_ael('exclusive($.hand == "hook")')
